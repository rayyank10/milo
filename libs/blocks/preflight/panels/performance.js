...
async function getResults() {
  ...
  ].filter((s) => s.value.icon === 'red').length;
  const warningCount = [
    lcpElResult, singleBlockResult, imageSizeResult, videoPosterResult,
    fragmentsResult, personalizationResult, placeholdersResult, iconsResult,
  ].filter((s) => s.value.icon === 'orange').length;
  window.dispatchEvent(new CustomEvent('preflight:badge', { detail: { tab: 'Performance', errors: errorCount, warnings: warningCount } }));
}
